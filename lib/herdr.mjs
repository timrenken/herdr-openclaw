...
export function isOpenClawPane(paneId, pane = null) {
  const title = pane?.terminal_title_stripped ?? pane?.terminal_title ?? "";
  if (/^openclaw-tui(?:$|\s)/i.test(title.trim())) return true;

  const info = processInfo(paneId);
  if (!info) return false;
  return (info.foreground_processes ?? []).some(
    (p) =>
      p.argv0 === OPENCLAW_ARGV0 ||
      /(^|[\\/])openclaw(?:\.cmd|\.exe)?$/.test(p.argv0 ?? "") ||
      /openclaw\.mjs.*\btui\b/i.test(p.cmdline ?? ""),
  );
}
