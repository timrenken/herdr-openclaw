...
// Some TUI builds add a human-readable session annotation after the stable id,
// e.g. `session main (mothy4286 user id:…) |`.  Keep the id separate: Herdr
// uses it as the session key, while the parenthetical text is display-only.
const INFO_LINE = /^\s*agent\s+([^|]+?)\s*\|\s*session(?:\s+(\S+)(?:\s+\([^)]*\))?)?\s*(?:\|\s*(.*))?$/;
